...
- **RevenueCat (iOS, added 2026-08-17):** project **`proj3cf3350c`** ("Duty"), app `appd65b7750dd` (App Store, `com.leftfieldapps.duty`), entitlement **`premium`** `entl7b4258f7e3`, offering `default` `ofrng4f5241a7f9` (packages `$rc_monthly`, `$rc_annual`). Public SDK key `appl_SvZiwbhFDoomDEvdFHDdTMSciig` is inlined in `src/lib/revenuecat.ts` (public by design). Secret key: `~/.config/gatheredin/revenuecat-duty.env`, never in the repo. **The RevenueCat `app_user_id` is the `duty_families.id`, not a user id** — premium is a family-level fact, so either parent's purchase covers the family. ASC subscription group `Duty Premium` 22316548; products `com.leftfieldapps.duty.premium.monthly` ($2.99) / `.annual` ($19.99). **Both Apple credentials ARE configured** in RevenueCat as of 2026-08-17: the App Store Connect API key (`424J7NT92Y`) and the In-App Purchase key (`BN52T4PN8D`, issuer `dff48c7f-f787-4bec-9f7c-def2559b6c58`, private key at `~/.appstoreconnect/private_keys/SubscriptionKey_BN52T4PN8D.p8`). Note Apple exposes NO API for creating an In-App Purchase key — it is dashboard-only — but RevenueCat's `POST /v2/projects/{p}/apps/{a}` accepts both keys, so uploading them needs no dashboard work.
- **`DUTY_REVENUECAT_WEBHOOK_SECRET` IS set** (2026-08-17) and the RevenueCat webhook integration `whintgr6516c4b04e` points at `https://isogetmvnpimcmouakeg.supabase.co/functions/v1/revenuecat-webhook` for **all environments and all event types** (so sandbox/TestFlight purchases fire it too). Verified end-to-end against the Demo Family: an `INITIAL_PURCHASE` wrote `premium_status=active` with a correct period end, and an out-of-order `EXPIRATION` was acknowledged but correctly did NOT revoke. Demo Family was restored to `free`/null/null afterwards.
- **Supabase management token** at `~/.config/gatheredin/supabase-mgmt.env` — this is how secrets get set now (`POST https://api.supabase.com/v1/projects/{ref}/secrets`); there is no MCP tool for secrets and the CLI needs an interactive login.
...
- **Push notifications — TWO transports.**
  - *Web / Home-Screen PWA:* Web Push, `DUTY_VAPID_*` secrets + client `VITE_VAPID_PUBLIC_KEY`.
  - *Native iOS App Store build:* **APNs** (iOS exposes Web Push only to Home-Screen web apps, never inside the Capacitor WKWebView). Uses `@capacitor/push-notifications`; `aps-environment=production` in `ios/App/App/App.entitlements`; AppDelegate forwards the two `capacitorDidRegister...` notifications. Server secrets: `DUTY_APNS_KEY_ID` (P4F5BY7BW2 — team-scoped key shared across Left Field apps), `DUTY_APNS_TEAM_ID`, `DUTY_APNS_PRIVATE_KEY`, `DUTY_APNS_BUNDLE_ID`, optional `DUTY_APNS_HOST` (set to `https://api.sandbox.push.apple.com` for dev-signed builds).
  - `duty_push_subscriptions` serves both: `platform='web'` rows carry endpoint/p256dh/auth, `platform='ios'` rows carry `device_token`.
